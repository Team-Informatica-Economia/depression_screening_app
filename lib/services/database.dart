...
Future<void> updateUser(Users users) {
  ...
  databaseReference
      .child("users")
      .child(users.uidPadre)
      .child("listaPazienti")
      .child(uid)
      .update(users.toJsonCompleto());
}
...
Future<Users> readUser() async {
  ...
  return new Users.overloadedConstructor(
      list[0]['nome'],
      list[0]['cognome'],
      list[0]['email'],
      list[0]["statoCivile"],
...
Future<List<Users>> readListPazienti() async {
  ...
  for (int i = 0; i < list.length; i++) {
    listaPazienti.add(new Users(list[i]['nome'], list[i]['cognome'],
        list[i]['email'], list[0]["uidPadre"]));
  }
...
Future<void> addPdfPaziente(Questionario questionario) async {
  ...
  databaseReference
      .child("users")
      .child(utenteLoggato.uidPadre)
      .child("listaPazienti")
      .child(uid)
      .child("listaQuestionari")
      .push()
      .set(questionario.toJson());
}
...
Future<List<Questionario>> readListQuestionari(Users paziente) async {
  ...
  for (int i = 0; i < list.length; i++) {
    listaQuestionari.add(new Questionario(
        list[i]['titoloPdf'], list[i]['path'], list[i]['punteggio']));
  }
  return listaQuestionari;
}

Future<void> addAppuntamento(
    String emailPaziente, AppuntamentoObj appuntamento) async {
  final User user = auth.currentUser;
  final uid = user.uid;
  DataSnapshot dataSnapshot = await databaseReference
      .child("users")
...
Future<AppuntamentoObj> readAppuntamento() async {
  ...
  Map<dynamic, dynamic> values = dataSnapshot.value;
  if (dataSnapshot.value != null) {
    obj = AppuntamentoObj(values["giorno"], values["mese"], values["anno"],
        values["giornoSettimana"], values["orario"]);
  }
  return (obj);
}
...
Future<List<Messaggio>> readListMessaggio() async {
  ...
  for (int i = 0; i < list.length; i++) {
    listaMessaggi.add(Messaggio(
        list[i]['messaggio'], list[i]['isPaziente'], list[i]['data']));
  }
